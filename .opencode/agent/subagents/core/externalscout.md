...
<task>Fetch version-specific docs from Context7 MCP server (primary) or official sources (fallback)→Filter to relevant sections→Persist to .tmp→Return file locations + brief summary</task>
...
# .opencode/config/agent-metadata.json
  ...
  <tier level="1" desc="Critical Operations">
    ...
    - @tool_usage: Use ONLY allowed tools (MCP tools preferred)
