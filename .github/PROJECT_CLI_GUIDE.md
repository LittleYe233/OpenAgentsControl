...
```bash
gh issue create \
  --repo LittleYe233/OpenAgentsControl \
...
make ideas
# or
gh issue list --repo LittleYe233/OpenAgentsControl --label idea
```
...
# or
gh issue view 123 --repo LittleYe233/OpenAgentsControl
```
...
# or
gh issue comment 123 --repo LittleYe233/OpenAgentsControl --body "Great idea!"
```
...
# or
gh issue close 123 --repo LittleYe233/OpenAgentsControl
```
...
```bash
make add-to-project ISSUE_URL=https://github.com/LittleYe233/OpenAgentsControl/issues/123
# or
gh project item-add 2 --owner darrenhinde --url https://github.com/LittleYe233/OpenAgentsControl/issues/123
```
...
**Create new label:**
```bash
gh label create "priority-high" --repo LittleYe233/OpenAgentsControl --color "d73a4a" --description "High priority"
```
...
```bash
gh issue edit 123 --repo LittleYe233/OpenAgentsControl --milestone "v0.2 - DX & Examples"
```
...
for i in 123 124 125; do
  gh issue close $i --repo LittleYe233/OpenAgentsControl
done
```
...
```bash
for i in 123 124 125; do
  gh issue edit $i --repo LittleYe233/OpenAgentsControl --add-label "priority-high"
done
```
...
- **Repository:** https://github.com/LittleYe233/OpenAgentsControl
...
3. **Use saved searches:**
   ...
   function oa-my-issues() {
     gh issue list --repo LittleYe233/OpenAgentsControl --assignee @me
   }
   ```
...
4. **Combine with git workflow:**
   ...
   ISSUE=$(gh issue create --repo LittleYe233/OpenAgentsControl --title "Fix bug" --label bug --format json | jq -r .number)
